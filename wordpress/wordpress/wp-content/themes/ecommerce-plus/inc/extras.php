 ...
 * @return array
 */
function ecommerce_plus_body_classes( $classes ) {
	...
	if ( is_singular() || is_home() ) {
		$id = ( is_home() && ! is_front_page() ) ? get_option( 'page_for_posts' ) : get_the_id();
	  	$sidebar = get_post_meta( $id, 'ecommerce-plus-selected-sidebar', true );
...
add_filter( 'body_class', 'ecommerce_plus_body_classes' );
