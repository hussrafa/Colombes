...
function ecommerce_plus_customize_register( $wp_customize ) {
	...
	require get_template_directory() . '/inc/customizer/theme-options/post.php';
	...
	$wp_customize->add_panel( 'ecommerce_plus_home_panel' , array(
	    'title'      => esc_html__( 'Home Page Sections','ecommerce-plus' ),
...
function ecommerce_plus_get_post_categories(){
	$cats = get_categories();
	$arr = array();
	$arr[''] = esc_html__('Select Post Category', 'ecommerce-plus');
	foreach($cats as $cat){
		$arr[$cat->term_id] = $cat->name;
	}
	return $arr;
