...
class Grocery_Store_Post_Related {
	/**
	...
	public function __construct() {
		...
		if( !is_admin()  )
		{
			add_action( 'grocery_store_site_content_type', array( $this,'site_loop_heading' ), 10 ); 
			add_action( 'grocery_store_site_content_type', array( $this,'site_content_type' ), 30 ); 
		}
		
		add_action( 'grocery_store_posts_blog_media', array( $this, 'render_thumbnail' ) ); 
		
		
		add_action( 'grocery_store_loop_navigation', array( $this,'site_loop_navigation' ) );
		add_action( 'grocery_store_single_post_navigation', array( $this,'single_post_navigation' ),10 ); 
    ...
    function site_content_type( ){
		...
		$type = apply_filters( 'grocery_store_content_type_filter', grocery_store_get_option( 'blog_loop_content_type' ) );
	...
	*/
	public function content_read_more_link( $more  ) {
		...
		return sprintf( '<div class="more-link">
             <a href="%1$s" class="btn  theme-btn inverted"><span>%2$s </span></a>
        </div>',
            get_permalink( get_the_ID() ),
			esc_html( grocery_store_get_option( 'read_more_text' ) )
        );
	...
	*/
	...
	function single_post_navigation( ) {
		...
		if( !empty($prevPost->ID) ):
			$img = wp_get_attachment_image_src( get_post_thumbnail_id( $prevPost->ID) , 'medium' );
			if( !empty($img[0]) ){
				$img_bg 	= "background:url(".esc_url( $img[0] ).") no-repeat center center; background-size:cover;";
				$image_have = 'img';
			}
		endif;

		$html .= '<div class="col-md-6 col-sm-6 text-right '.esc_attr($image_have).'" style="'.esc_attr($img_bg).'">';
		if( $prevPost ) :
			
				$html .='<h5>'.esc_html__('Previous Article','grocery-store').'</h5>';
				$html .='<h4>'.get_previous_post_link('%link',"<span>%title</span>").'</h4>'; 
		...
		if( !empty($nextPost->ID) ):
			$img = wp_get_attachment_image_src( get_post_thumbnail_id( $nextPost->ID) , 'medium' );
			if( !empty($img[0]) ){
				$img_bg 	= "background:url(".esc_url( $img[0] ).") no-repeat center center; background-size:cover;";
				$image_have = 'img';
			}
		...
		$html .= '<div class="col-md-6 col-sm-6 text-left '.esc_attr($image_have).'" style="'.esc_attr($img_bg).'">';
		if( $nextPost ) : 

				$html .='<h5>'.esc_html__('Next Article','grocery-store').'</h5>';
				$html .= '<h4>'.get_next_post_link('%link',"<span>%title</span>").'</h4>';
		...
		$html  	 = apply_filters( 'grocery_store_single_post_navigation_filter', $html );
	...
	function site_loop_navigation( $type = '' ) {
		...
		if( $type == '' ){
			$type = apply_filters( 'grocery_store_loop_navigation_filter', get_theme_mod( 'grocery_store_loop_navigation', 'default' ) );
		}
		...
		if( $type == 'default' ):
			...
			the_posts_navigation(
				array(
					'prev_text' => '<i class="icofont-rounded-double-left"></i><span>'.esc_html__('Prev Posts', 'grocery-store').'</span>',
					'next_text' => '<span>'.esc_html__('Next Posts', 'grocery-store').'</span><i class="icofont-rounded-double-right"></i>',
					'screen_reader_text' => __('Posts navigation', 'grocery-store')
				)
			);
			...
			the_posts_pagination( array(
				...
				'type' => 'list',
				'mid_size' => 2,
				'prev_text' => esc_html__( 'Previous', 'grocery-store' ),
				'next_text' => esc_html__( 'Next', 'grocery-store' ),
				'screen_reader_text' => esc_html__( '&nbsp;', 'grocery-store' ),
			) );
		echo '</div>';
		endif;
	...
	public function get_gallery_thumbnail(){
		...
		$html =  apply_filters( 'grocery_store_gallery_thumbnail', $html );
	...
	public function get_audio_thumbnail(){
		...
		$html =  apply_filters( 'grocery_store_audio_thumbnail', $html );
	...
	public function get_video_thumbnail(){
		...
		$html =  apply_filters( 'grocery_store_video_thumbnail', $html );
	...
	public function get_image_thumbnail(){
		...
		$html =  apply_filters( 'grocery_store_image_thumbnail', $html );
...
new Grocery_Store_Post_Related();