...
class Grocery_Store_Post_Meta {
	...
	public function __construct() {
		...
		add_action( 'grocery_store_do_tags', array( $this, 'render_tags_list' ) );
		add_action( 'grocery_store_site_content_type', array( $this, 'render_meta_list' ), 10, 1 );
		add_action( 'grocery_store_meta_info', array( $this, 'render_meta_list' ), 10, 2 );
	...
	public function render_meta_list( $order, $wrp_class = '' ) {
		...
		foreach ( $order as $meta ) {
			switch ( $meta ) {
				case 'author':
					...
					$markup .= ' <span>'. esc_html__( 'By -','grocery-store' ) .'</span>';
				...
				case 'date':
					...
					$markup .= esc_html__( 'Posted on ','grocery-store' );
					$markup .= $this->get_time_tags();
					$markup .= '</li>';
					break;
				case 'category':
					$markup .= '<li class="meta category">';
					$markup .= esc_html__( 'Posted in ','grocery-store' );
		...
		echo wp_kses( $markup, grocery_store_alowed_tags() );
	}
	...
	private function get_comments() {
		...
		if ( $comments_number == 0 ) {
			...
			$comments = sprintf( _n( '%s Comment', '%s Comments', $comments_number, 'grocery-store' ), $comments_number );
		}
	...
	public function render_tags_list() {
		...
		$html .= '<span>' . esc_html__( 'Tags', 'grocery-store' ) . ':</span>';
		foreach ( $tags as $tag ) {
...
$grocery_store_meta = new Grocery_Store_Post_Meta();