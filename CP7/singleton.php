...
class Singleton
{
    ...
    /**
     * Methode qui renvoie le resultat d'une reauete 
     * sous la forme d'un objet JSON
     * @param string $sql - requete SELECT/SHOW
     * @param array $vals - paramétres
     */
    public static function getAllData(string $sql, array $vals = []): string
    {
        try {
            // Prépare la requête
            $qry = self::getPDO()->prepare($sql);
